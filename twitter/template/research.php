<?php
    require "connexion.php";
        if (isset($_GET['research'])) {
            $query = $_GET['research'];
            $requete = $database->prepare("SELECT * FROM tweets INNER JOIN users ON tweets.user_id = users.users_id WHERE tweet LIKE :query ORDER BY date DESC");
            $requete->execute([
                ':query' => '%'.$query.'%'
            ]);
    ...
    <h2 class="titleResult" >Résultats de recherche pour "<?php echo $query; ?>"</h2>
        <?php if (count($results) > 0): ?>
            <?php foreach ($results as $element){  ?>
                <div class="newAdd" id="tweetSearch<?php echo $element['tweets_id']; ?>">
                        <h4><?php echo $element['pseudo']; ?></h4>
                            <p><?php echo $element['tweet']; ?></p>
                        <h6><?php echo $element['date']; ?></h6>
                    <form action="delete.php" method="POST">
                        <input type="hidden" name="supp" value="<?php echo $element['tweets_id']; ?>">
